...
public class ParticipantManager {
    ...
    public void handleNewSession() throws Exception {
         ...
         */
        for (final PreConnectCallback callback : _preConnectCallbacks) {
    ...
    private void createLiveInstance() {
        final String liveInstancePath = _keyBuilder.liveInstance(_instanceName).getPath();
        ...
        if (_liveInstanceInfoProvider != null) {
            ...
            final ZNRecord additionalLiveInstanceInfo =
            ...
            if (additionalLiveInstanceInfo != null) {
                ...
                final ZNRecord mergedLiveInstance = new ZNRecord(additionalLiveInstanceInfo, _instanceName);
        ...
        do {
            ...
            try {
                ...
                LOG.info("LiveInstance created, path: " + liveInstancePath + ", sessionId: " +
                ...
                if (record == null) {
                    ...
                    final String ephemeralOwner = Long.toHexString(stat.getEphemeralOwner());
                    if (ephemeralOwner.equals(_sessionId)) {
                        /**
                         * update sessionId field in live-instance if necessary
                         */
                        final LiveInstance curLiveInstance = new LiveInstance(record);
    ...
    private void carryOverPreviousCurrentState() {
        final List<String> sessions = _dataAccessor.getChildNames(_keyBuilder.sessions(_instanceName));
        ...
        for (final String session : sessions) {
            ...
            for (final CurrentState lastCurState : lastCurStates) {
                ...
                final StateModelDefinition stateModel =
                        _dataAccessor.getProperty(_keyBuilder.stateModelDef(stateModelDefRef));

                final BaseDataAccessor<ZNRecord> baseAccessor = _dataAccessor.getBaseDataAccessor();
                final String curStatePath =
                        _keyBuilder.currentState(_instanceName, _sessionId, lastCurState.getResourceName())
                                .getPath();

                final String initState = stateModel.getInitialState();
    ...
    private void setupMsgHandler() throws Exception {
        ...
        ScheduledTaskStateModelFactory stStateModelFactory = new ScheduledTaskStateModelFactory(
